//CLIENTE.C - ANDRÉ HENRIQUES RODRIGUES [2016057941] & LUCIANO OTONI MILEN [2012079754]
...
int getBuff(char *buffer, int buffSize) {
        ...
        while (getPackID(pack) != packID) {
                createPack(ACKMSG, getPackID(pack), ACK_TYPE, HEADER + pack,
                           buffSize); // cria um novo pacote com o ID do último recebido
...
int main(int argc, char *argv[]) {
        ...
        if (argc < 6) {
                printf("Usage: ./cliente <serverHost> <serverPort> <fileName> <buffSize> <window_size>\n");
                exit(1);
        }
        // aloca memorias...
